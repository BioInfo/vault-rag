...
def create_index(documents: List[Document], vector_store: ChromaVectorStore) -> VectorStoreIndex:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Ingest markdown files into vector database')
    parser.add_argument('--vault', default=config.VAULT_PATH, help='Path to vault directory')
    parser.add_argument('--chunk-size', type=int, default=config.CHUNK_SIZE, help='Chunk size for splitting')
    parser.add_argument('--chunk-overlap', type=int, default=config.CHUNK_OVERLAP, help='Chunk overlap')
    parser.add_argument('--collection', default=config.COLLECTION_NAME, help='ChromaDB collection name')
